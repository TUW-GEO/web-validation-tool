...
from geo_python.RS.Validation.io.db_model import interface, Location, ERA_Interim_db, Ascat_ssm
import geo_python.general.grid.dgg.find as find_gp
import geo_python.RS.dataspecific.ECMWF.grid as era_grid
from geo_python.RS.WARP.io.interface import METOP_WARP55R11
from geo_python.RS.dataspecific.ECMWF.io.interface import ERA_Interim
...
def add_stations_for_ismn_network(network_abbr):
    ...
    Stations = ISMN.get_stations_with_data_between(
        datetime(2007, 01, 01), datetime(2013, 2, 28), network=Network)
    ...
    for station in Stations:
        ...
        warp_gpi, warp_dist, warp_lat, warp_lon, warp_status = find_gp.find_nearest_gp(
            station.lat, station.lon, "warp_grid", lm_name='ind_ld')
        if warp_status == 0:
        ...
        if not warp_gpi[0] in added_warp_gpis:
            ...
            for i, value in enumerate(ssm.index):
                vt_db.session.add(Ascat_ssm(warp_gpi[0], ssm.index[i], ssm['sm'].values[i], ssf[
                                  'ssf'].values[i], warp_version, direction[ssm['dir'].values[i]]))
