jQuery.fn.bulletGraph = function(settings) {
    ...
    var options = jQuery.extend({
        ...
        sliderOptions: null,
		// custom colors are disabled by default
		customColors:true,
    ...
    jQuery.each(ranges,
    function(i, v) {
        var curRange = jQuery('<div title="'+options.rangesTitles[i]+'"></div>');
		...
		if(options.customColors){
			...
			var color1=options.colors[i]
			var color2=options.colors[i]
    ...
    if (target != undefined) {
        var curTarget = jQuery('<div id="target"></div>');
	...
	// if legend should be output
    if (options.showLegend) {
	
		var curMark = jQuery('<div></div>');
                curMark.addClass('legend');
				curMark.append(options.legendTitle);
				curMark.css('top', options.height + 7+10);
                box.append(curMark);
	}
