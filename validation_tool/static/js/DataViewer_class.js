...
ISMNDataViewer.prototype.loadVariableList = function() {
        ...
        $.getJSON('./dataviewer_get_variable_list', {
            station_id: this.selectedStation.id,
            start: $("#date_from").prop('value'),
            end: $("#date_end").prop('value')
        }, function(data) {
...
ISMNDataViewer.prototype.displayVariable = function(variableIndex) {
    ...
    if (this.loaded[variableIndex] !== true) {
        $('#' + this.loadingDiv).removeClass('hidden');
        $.getJSON('./dataviewer_load_variable', {
                station_id: this.selectedStation.id,
                start: this.minTime,
                end: this.maxTime,
...
ISMNDataViewer.prototype.loadPossibleTimestamps = function() {
    ...
    $.getJSON('./dataviewer_load_possible_timestamps', {
        station_id: this.selectedStation.id,
        start: this.minTime,
        end: this.maxTime
    }, function(data) {
...
ISMNDataViewer.prototype.showVariable = function(variableIndex) {
    ...
    if (!this.displayed[variableIndex]) {
        ...
        if (this.data.length === 0) {
            ...
            if (this.graph === null) {
                this.graph = new Dygraph(document.getElementById(this.dygraphDiv), [
                    [0, 0],
                    [0, 1]
                ], {
                    showRoller: true,
